...
namespace Poke.Logic.Scene
{
    public class Arena : Node2D
    {
        ...
        set
        {
            ...
            texture.Load("res://Images/Pokes/" + value.ImageFileTitle);
        ...
        public override void _Ready()
        {
            ...
            // Генерируем рандомные Id игрока и оппонента
            int opp = new Random().Next(1,4);
        ...
        void LoadButtons()
        {
            ...
            {
                var btn = new AttackButton() {RectSize = new Vector2(20,50), Text=$"button №{i}"};
                btn.Name = $"Btn_Atk{i}";
                btn.Text = Player.Attacks[i].Title;
                btn.RectMinSize = new Vector2(125,25);
                btn.SetMeta("AtkNum",i);
                btn.SetMeta("sender",this);
                // var btid = btn.GetInstanceId();
                // list.AddChild((Node)GD.InstanceFromId(btid));
                list.AddChild(btn);
